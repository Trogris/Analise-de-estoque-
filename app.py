...
def aplicar_regras_com_alertas(estrutura, estoque, destino, qtd_equipamentos):
    ...
    for _, row in estrutura_group.iterrows():
        ...
        if status != "Ok":
            ...
            elif codigos['OI'] >= falta:  # OI pode ser usado diretamente
                status = f"🟢 Ok (usar {int(falta)} de OI)"
            else:
    ...
    if not {'Item', 'Quantidade'}.issubset(estrutura.columns):
        ...
        if st.button("✅ Executar Análise"):
            with st.spinner("Analisando os dados..."):
                ...
                with col1:
                    # Métrica 1: Total analisados + Percentual em estoque
                    st.markdown(f"""
                ...
                # Seção adicional para unidades e custo
